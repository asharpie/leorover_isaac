...
def main():
    ...
    print(f"{'H':>5} {'hyb_succ%':>10} {'lqr_succ%':>10} {'hyb_CTE':>9} {'lqr_CTE':>9} "
          f"{'CTEgap%':>8} {'hyb_park%':>10} {'lqr_park%':>10} {'eps':>6}")
    for h, d, hy, lq in pts:
        gap = 100.0 * (hy["cte"] - lq["cte"]) / lq["cte"] if lq["cte"] else float("nan")
        print(f"{h:5.2f} {hy['succ']:10.1f} {lq['succ']:10.1f} {hy['cte']:9.3f} {lq['cte']:9.3f} "
              f"{gap:8.1f} {hy['park']:10.1f} {lq['park']:10.1f} {min(hy['n'], lq['n']):6d}")

    def collapse_h(key):
        for h, _, hy, lq in pts:
            if (hy if key == "hyb" else lq)["succ"] < 5.0:
                return h
        return None

    ch, cl = collapse_h("hyb"), collapse_h("lqr")
    ...
    alive = [h for h, _, hy, lq in pts if max(hy["succ"], lq["succ"]) >= 5.0]
    if alive:
