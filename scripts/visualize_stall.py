...
def main():
    ...
    order = [int(i) for i in np.argsort(finalprog)[::-1] if int(i) != e]
    if order:
        es = order[0]   # best-MOVING rover in this window (highest progress)
        pes = P[:, es, :, :] - origin[es][None, None, :]
        wzs = pes[:, wheel_ids, 2]
        sp_p = float(wz[-1].max() - wz[-1].min())
        sp_s = float(wzs[-1].max() - wzs[-1].min())
        print(f"\n[viz] CONTRAST  parked env {e} (prog {finalprog[e]:.0f}%) spread {sp_p:.3f} m"
              f"   vs   mover env {es} (prog {finalprog[es]:.0f}%) spread {sp_s:.3f} m")
        print(f"[viz]   mover wheel heights {[round(float(wzs[-1,j]),3) for j in range(len(wheel_ids))]}")
        print(f"[viz]   => parked spread >> mover spread means parked rovers settle TILTED "
              f"(rocking on the wheel/terrain contact).", flush=True)
