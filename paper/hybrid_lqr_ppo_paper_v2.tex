...
\begin{itemize}
  ...
  \item An empirical characterization of \emph{when residual learning pays off}: on rigid contact the hybrid's gains are small and confined to agility-limited cells; on deformable soil they are large ($-47\%$ cross-track error) and hold at every difficulty level, indicating that contact fidelity, not reward shaping, was the binding constraint. A matched-budget end-to-end PPO ablation pair sharpens the comparison: an ungated progress reward converges to counterfeit completion (off-path shortcuts), a corridor-gated reward to honest but narrow tracking (a $13\%$ difficulty ceiling), so at this budget only the residual architecture attains breadth and path fidelity together.
...
\vspace{2mm}
\noindent\textbf{Model-free DRL and large-scale simulation.} Model-free DRL has been successfully applied to complex locomotion and off-road tasks \cite{wiberg2021rough, gunma2024sac, lee2020learning}. A key enabler has been GPU-parallel simulation: Isaac Gym \cite{makoviychuk2021isaac} and the Orbit/Isaac Lab framework \cite{mittal2023orbit} run thousands of environments on a single GPU, and Rudin et al.\ \cite{rudin2022learning} showed that a game-inspired terrain curriculum with massive parallelism can train quadruped locomotion in minutes. These advances directly motivate our use of Isaac Lab. Three limitations remain for our setting: simulator fidelity for granular soils, partial observability of slip \cite{alcayaga2025lstm}, and, as our results demonstrate, objective anchoring, since scale alone does not prevent an end-to-end policy from optimizing a tracking reward by shortcutting the path it is meant to track. We address the first with an explicit soil-force layer, the second by making per-wheel slip and sinkage part of the observation, and the third structurally, with the LQR baseline itself.
...
\end{enumerate}
\noindent\emph{PPO baseline status.} With the stability measures of Section~III-C (noise-std clamp) and its ramped terrain curriculum, the end-to-end PPO baseline trained to genuine competence on deformable soil: $30{,}000$ updates ($3.93\times10^{9}$ environment steps, slightly \emph{more} than the hybrid's budget) with monotone learning. Notably, its curriculum ceiling froze at $31\%$ terrain difficulty for the entire second half of training: the automatic curriculum advances only when tracking quality passes a cross-track gate, and PPO's deterministic CTE plateaued at $0.154$~m, so the curriculum itself diagnosed a tracking deficit long before evaluation did (Section~\ref{sec:sandresults}).

\subsection{Simulation Experiments}
\subsubsection{Matched-Condition (Scenario-Locked) Comparison}
A central design choice is that controllers are evaluated on \emph{exactly the same} episodes. For the paired suites, a scenario file pre-draws, for each of $90{,}000$ episode slots: the reference path, the terrain patch assignment, and the start pose; the seeded soil field is identical by construction; and each scenario carries its own per-wheel friction values, applied at reset in place of the live randomization, so that even traction is reproduced exactly across separately launched runs (a fixed-friction mode is retained for controlled sweeps). Each controller then consumes the identical scenario list, so any difference in outcome is attributable to the controller alone, and paired statistics (paired $t$ on per-scenario cross-track error, McNemar on per-scenario success outcomes, Cohen's $d_z$, Bonferroni-corrected) are valid. Success is \emph{progress-gated}: an episode counts as successful only if the rover reaches the final waypoint within tolerance \emph{and} has traversed at least $90\%$ of the path. The gate matters: for closed polygon paths the goal coincides with the start, and without the gate a stationary controller scores $100\%$ instantly, an artifact that inflated early evaluations and that we flag for practitioners.
...
\begin{table}[t]
\centering
\caption{Deformable Soil, Randomized-Terrain Suite: Success and MCTE by Terrain Level (Deterministic Policies, Per-Episode Random Friction; Three-Way Replicate, $n \approx 12$--$13$k per Controller)}
\label{tab:sand_levels}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}lcccccc@{}}
\toprule
& \multicolumn{2}{c}{\textbf{LQR}} & \multicolumn{2}{c}{\textbf{Hybrid}} & \multicolumn{2}{c}{\textbf{PPO}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(l){6-7}
\textbf{Level} & Succ.\ (\%) & MCTE (m) & Succ.\ (\%) & MCTE (m) & Succ.\ (\%) & MCTE (m) \\
\midrule
0 (flat) & 86.5 & 0.139 & 93.8 & \textbf{0.038} & 100.0 & 0.087 \\
20 & 98.2 & 0.050 & 98.4 & \textbf{0.041} & 99.9 & 0.082 \\
40 & 81.7 & 0.165 & 81.6 & \textbf{0.097} & 85.9 & 0.141 \\
60 & 79.9 & 0.113 & 80.8 & \textbf{0.085} & 82.0 & 0.144 \\
80 & 53.9 & 0.176 & 55.6 & \textbf{0.143} & 57.4 & 0.209 \\
100 & 75.2 & 0.164 & 80.7 & \textbf{0.094} & 85.9 & 0.163 \\
\midrule
\textbf{Overall} & 79.3 & 0.134 & 81.9 & \textbf{0.083} & 85.5 & 0.137 \\
\bottomrule
\end{tabular}%
}
\\[2pt]
{\footnotesize PPO's completion column reflects relaxed path adherence (corner-cutting); see the tracked-completion analysis in the text. An earlier two-controller run of this suite gave Hybrid $83.4\%/0.073$ vs.\ LQR $79.3\%/0.137$; hybrid/LQR figures replicate within run-to-run episode-draw variation.}
\end{table}

\vspace{1mm}
\noindent\textbf{End-to-end PPO: completion through path relaxation.} The trained PPO baseline posts the \emph{highest} raw completion in Table~\ref{tab:sand_levels} ($85.5\%$, including $100\%$ on flat sand and $85.9\%$ at maximum difficulty, far beyond its $31\%$ training ceiling), and the mechanism is identified by three converging measurements: its successful traversals are as fast or faster than the others (median $58$~s vs.\ $67$~s on flat, i.e.\ it drives a \emph{shorter line than the reference path}), its realized slip is the lowest of the three ($0.364$ vs.\ $0.398$--$0.405$), and its tracking error is the worst ($0.137$~m; $0.087$~m even on unobstructed flat ground). Free of the LQR reference, the progress-dominant reward taught PPO to smooth and shortcut the path: beelining between waypoints avoids the continuous steering that generates slip, which simultaneously explains the high completion, the low slip, the speed, the poor tracking, and the difficulty-agnostic generalization (a shortcut policy transfers because it does not engage the terrain aggressively anywhere). This is goal-reaching, not path-tracking. Defining \emph{tracked completion} as reaching the goal with episode MCTE $\le 0.10$~m, the ranking inverts decisively: hybrid $77.1\%$, LQR $71.8\%$, PPO $53.7\%$, with the hybrid leading at every terrain level (flat: $93.0 / 80.1 / 74.1$; maximum difficulty: $73.8 / 66.5 / 47.2$). Under the joint criterion the mission actually requires, the hybrid is the only controller that is simultaneously accurate, fast, and reliable, and end-to-end PPO's apparent completion advantage is revealed as $31.8$ points of arriving off-route.
...
\vspace{1mm}
\noindent\textbf{Held-out worlds (generalization).} To test whether the advantage depends on the specific training terrain, we regenerated the entire world (all $12$ terrain patches and the soil-softness field) from fresh master seeds and re-evaluated the same trained policy. A first single-world probe illustrated the seed variance this test must contend with: under randomized friction, tracking transferred in full (MCTE $0.134$ vs.\ $0.177$~m, $-24\%$, lower at every level; flat sand $0.044$ vs.\ $0.166$~m) but completion reached only parity, because that world's maximum-difficulty row happened to realize roughly twice the training world's relief (mean spawn-origin height $0.97$~m vs.\ $0.51$~m) and collapsed both controllers equally there (success $\approx 22\%$); below that saturated row, success matched the training world ($84.4\%$ vs.\ $84.9\%$). With two patch variants per row, a single draw can define a level, and at that roughness the failure mode is geometric infeasibility, which no bounded velocity residual can overcome.

The formal generalization test is therefore a \emph{sixteen-world paired suite}: the scenario-locked protocol repeated over $16$ fully regenerated worlds ($192$ distinct terrain patches, $16$ soil fields), with one shared scenario list so that episode $j$ is identical for both controllers in every world. Diversity is maximized within the pairing: paths are drawn from a $2{,}000$-path bank (training saw only $500$, so three quarters of the evaluation paths are unseen), and each scenario carries its \emph{own} per-wheel friction (a per-episode base $\mu \sim \mathcal{U}[0.47, 2.0]$ times $\pm15\%$ per-wheel jitter), stored in the scenario file and applied at reset, so even traction is byte-identical across controllers, verified at a $100.0\%$ per-scenario friction match between legs. Over $n = 185{,}410$ matched pairs, the hybrid achieves lower MCTE in $16$ of $16$ worlds (sign-test $p = 3\times10^{-5}$; pooled $0.117$ vs.\ $0.140$~m, $-16\%$; paired $t = -67.0$, $d_z = 0.16$) and higher success in $15$ of $16$ (sign-test $p = 5\times10^{-4}$; pooled $77.9\%$ vs.\ $77.3\%$; McNemar $\chi^2 = 105$, $p < 10^{-4}$), and leads on both metrics at every terrain level pooled across worlds (Table~\ref{tab:multiworld}), with the largest per-level tracking gap on flat sand ($0.042$ vs.\ $0.096$~m, $-56\%$). An initial fixed-friction variant of the suite ($\mu = 1.0$ everywhere) gave the same verdict ($16/16$ tracking, $14/16$ completion), so neither result hinges on the friction protocol.

Because friction is now a paired, logged variable, the suite also yields the cleanest mechanism evidence in this paper (Table~\ref{tab:multiworld}, lower panel; Figure~\ref{fig:dose_response}): conditioned on traction, the tracking gap is a monotone dose-response, from \emph{exactly zero} on high-grip episodes ($0.053$ vs.\ $0.053$~m), through $-9\%$ at moderate grip, to $-21\%$ ($0.250$ vs.\ $0.315$~m, with $+1.4$ success points) under scarce traction. The residual is selectively active precisely where the linear baseline's model breaks down, and its effort penalty keeps it silent where LQR is already near-optimal; this also quantifies how the advantage scales toward the $-47\%$ observed on the training world, whose remaining margin over the held-out $-16\%$ we attribute to familiarity with the training world's specific soil layout and path set. The conclusion is that the slip-management behavior generalizes across worlds, paths, and traction levels on both metrics, with magnitude governed by how much slip the episode induces.

\begin{table}[t]
\centering
\caption{Sixteen Held-Out Worlds, Paired Suite ($185{,}410$ Matched Pairs; Scenario-Locked Per-Wheel Friction $\mu \in [0.47, 2.0]$): Pooled Success and MCTE by Terrain Level and by Traction}
...
& $n$ & Succ.\ (\%) & MCTE (m) & Succ.\ (\%) & MCTE (m) \\
\midrule
\multicolumn{6}{c}{\textit{By terrain level (\%)}} \\
0 (flat) & 30{,}084 & 90.2 & 0.096 & \textbf{92.6} & \textbf{0.042} \\
20 & 31{,}998 & 90.5 & 0.101 & \textbf{90.6} & \textbf{0.083} \\
40 & 30{,}859 & 84.8 & 0.121 & 84.8 & \textbf{0.105} \\
60 & 30{,}961 & 74.3 & 0.150 & \textbf{74.7} & \textbf{0.134} \\
80 & 31{,}027 & 61.7 & 0.172 & \textbf{61.9} & \textbf{0.158} \\
100 & 30{,}481 & 62.0 & 0.199 & \textbf{62.8} & \textbf{0.181} \\
\midrule
\multicolumn{6}{c}{\textit{By traction (friction intensity)}} \\
Low ($<40$) & 58{,}457 & 56.8 & 0.315 & \textbf{58.2} & \textbf{0.250} \\
Mid ($40$--$70$) & 62{,}384 & 87.1 & 0.066 & \textbf{87.5} & \textbf{0.060} \\
High ($\ge 70$) & 64{,}569 & 86.4 & 0.053 & \textbf{86.6} & 0.053 \\
\midrule
\textbf{Pooled} & 185{,}410 & 77.3 & 0.140 & \textbf{77.9} & \textbf{0.117} \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_friction_dose.pdf}
\caption{Traction dose-response on the sixteen-world paired suite: MCTE and success versus scenario-locked wheel friction (bin centers, $n \approx 28$k--$33$k pairs per bin). The controllers are indistinguishable at high grip; the hybrid's advantage grows monotonically as traction falls, exactly where the linear baseline's model breaks down.}
\label{fig:dose_response}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_multiworld_forest.pdf}
\caption{Per-world paired differences across the sixteen held-out worlds ($95\%$ confidence intervals). Every $\Delta$MCTE interval lies strictly below zero; $\Delta$success is positive in $15$ of $16$ worlds.}
\label{fig:multiworld_forest}
\end{figure}
...
Completion on this suite requires a fair clock before it can be interpreted at all. A single global episode cap under-times long geometries: at the training cap ($400$~s) a successful zig-zag median traversal alone takes $262$~s, and both controllers' completion is cap-dominated ($22.2\%$ vs.\ $25.3\%$ at fixed friction). We therefore made \emph{path-proportional time budgets} the primary protocol: every episode receives $40$~s per meter of reference path, exactly the per-meter allowance the $10$~m training paths have always had (so training and the randomized suite are unchanged by construction), with the longest ($\approx 31$~m) geometries receiving $\approx 1255$~s. Under fair clocks, no rover that can finish is denied the time to do so, and the failure anatomy confirms it: failed episodes terminate at less than half their budget (median failed zig-zag ends at step $2661$ of $\approx 5800$ available, at $36\%$ progress) through the stagnation detector, i.e.\ by \emph{bogging at hairpins}, while successful traversals finish with thousands of steps to spare. Completion on this suite now measures traversal ability, not speed. Two fixed-cap variants ($400$~s and $800$~s) serve as the sensitivity analysis; across all three protocols the ordering is invariant.

Under the fair-clock protocol ($n = 89{,}499$ matched scenarios, scenario-locked friction), the hybrid tracks better than LQR in every geometry and slope cell (MCTE $0.4225$ vs.\ $0.4705$~m; paired $t = -73.9$, $d_z = 0.25$) while LQR completes somewhat more ($32.5\%$ vs.\ $29.0\%$; McNemar $\chi^2 = 940$, discordant $6832/3686$). The mechanism is now located precisely: on hairpin geometry in sand, \emph{tight tracking is itself a slip hazard}. LQR's wider, momentum-carrying zig-zag lines ($0.568$~m MCTE there) bog slightly less often than the hybrid's tighter ones ($0.518$~m; completion $25.5\%$ vs.\ $21.6\%$), a genuine tracking-versus-entrapment tension on extreme geometry that persists with time removed, and we report it as the honest boundary of the hybrid's advantage. The randomized suite, whose paths match the training distribution, shows the hybrid ahead on \emph{both} metrics at every level (Table~\ref{tab:sand_levels}).

The three-way fair-clock suite adds the decisive PPO result: the shortcut policy that led raw completion on the randomized suite \emph{collapses} on adversarial reference geometry, completing $20.2\%$ overall and $0.0\%$ of zig-zags, a number the fair clock makes unambiguous: given roughly three times the original cap, PPO completes not a single zig-zag out of thousands, with median progress $9.8\%$ while consuming its budget in place. It cannot execute the maneuver; time was never the constraint. (Its polygon completion does rise to $16.6\%$, matching LQR: smooth closed circuits suit a shortcut policy, sharp reversals do not.) This also produces a statistical trap we flag for practitioners: PPO's raw per-episode MCTE on this suite ($0.278$) is the \emph{lowest} of the three, because error is only accumulated over the small path fraction the policy engages; the paired MCTE test naively favors PPO ($d_z = 0.62$) while measuring a censored quantity. Conditioning on the scenarios that both controllers actually complete removes the censoring and reverses the verdict: hybrid $0.094$ vs.\ PPO $0.133$~m ($n = 14{,}475$ jointly completed), a $29\%$ hybrid advantage, and on completion the paired comparison is one-sided (McNemar $\chi^2 = 4111$, discordant $11464/3595$ in the hybrid's favor, $p < 10^{-4}$). Cross-controller MCTE comparisons on suites with heterogeneous completion should always be accompanied by the jointly-completed conditioning.

\vspace{1mm}
\noindent\textbf{Hardening the PPO reward: the corridor-gated ablation.} A fair objection to the preceding analysis is that the shortcut optimum indicts the reward as much as the architecture, so we retrained end-to-end PPO with the standard path-following remedy: progress credit multiplied by a tracking kernel, $r_{\text{prog}} \mathrel{*}= \exp(-(e_{\text{lat}}/0.20\,\text{m})^2)$, so beelining earns nothing (a standard multiplicative tracking kernel in path-following RL; all other settings unchanged; identical $3.93\times10^9$-step budget). The gate works as intended, and the result completes the picture. The shortcut disappears, and the policy learns \emph{genuine} tracking: on flat sand, its trained regime, it completes $83.6\%$ with $0.061$~m MCTE, tracking \emph{better than pure LQR} ($0.113$~m) on the same terrain. But its curriculum froze at $13\%$ difficulty for the entire run, blocked by the same $0.10$~m tracking gate that capped the ungated variant at $31\%$ (final state: success $63\%$, CTE $0.106$~m, six millimeters short of advancing), and on the full difficulty sweep it collapses to $27.1\%$ completion with $14.5\%$ of episodes parked, failing above its ceiling in the crawl mode (median progress $18$--$40\%$ while consuming the full episode). The two runs therefore bracket end-to-end PPO's option space at this budget: an ungated objective yields broad but counterfeit completion (the shortcut policy), a gated objective yields honest tracking with a drastically narrowed competence envelope (the crawl policy), and both stall against the identical tracking-quality gate. The gated run is, to our knowledge, the most favorable evidence for pure RL in this study, because it shows the \emph{capability} is learnable: RL can out-track a well-tuned linear controller on deformable soil. What it cannot do, in either reward variant, is extend that capability across the difficulty range, and that reach is precisely what the LQR anchor provides the hybrid for free.
...
\item Sixteen held-out worlds, paired (per-wheel friction scenario-locked): MCTE lower in $16/16$ worlds (sign-test $p = 3\times10^{-5}$); pooled $t = -67.0$, $d_z = 0.16$, $n = 185{,}410$.
\item Rigid control condition: significant but small ($d_z \approx 0.05$; Section~\ref{sec:rigidresults}).
\item Deformable soil, fair-clock paired suite (path-proportional budgets): $t = -73.9$, $d_z = 0.25$, $n = 89{,}499$, every geometry and slope cell.
\item Hybrid vs.\ PPO: randomized suite (same-harness) $0.083$ vs.\ $0.137$~m; fair-clock paired suite with jointly-completed conditioning, $0.094$ vs.\ $0.133$~m ($n = 14{,}475$); the raw paired statistic on that suite is censoring-confounded by PPO's low median progress and is reported only with that caveat.
\end{itemize}
...
\item Sixteen held-out worlds, paired: success higher in $15/16$ worlds (sign-test $p = 5\times10^{-4}$); pooled $77.9\%$ vs.\ $77.3\%$, McNemar $\chi^2 = 105$, $p < 10^{-4}$.
\item Deformable soil, discrete suite: LQR completes more in every protocol ($\chi^2 = 863$ at the training cap; $\chi^2 = 940$, discordant $6832/3686$, under fair path-proportional clocks), so the hairpin tracking-versus-entrapment trade is intrinsic and time-independent; reported without adjustment.
\item Hybrid vs.\ PPO, fair-clock paired suite: $\chi^2 = 4111$, discordant $11464/3595$, $p < 10^{-4}$ in the hybrid's favor (PPO completes $20.2\%$, including $0.0\%$ of zig-zags despite path-proportional budgets).
...
\vspace{1mm}
\noindent\textbf{Learning dynamics (RQ2).} The hybrid trained from LQR-level competence ($62\%$ deterministic success) to $80$--$81\%$ in a single run of $3.3\times10^9$ steps; training was monotone after the stability measures of Section~III-C (std clamp; effort weight $0.01$; credit term), with the action-noise std self-regulating at $0.06$--$0.08$ (Figure~\ref{fig:learning_curves}). The end-to-end PPO baseline also trained stably ($30{,}000$ updates, $3.93\times10^9$ steps, deterministic success $93\%$ at its curriculum ceiling), but its curriculum froze at $31\%$ difficulty because its tracking never passed the $0.10$~m CTE advance gate (plateau at $0.154$~m), and its noise std pinned at the clamp floor. The two runs jointly answer RQ2 for tracking: comparable budgets produce a path-tracking controller with the residual structure and a goal-reaching shortcut policy without it.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_train_world_levels.pdf}
\caption{Success rate and mean cross-track error by terrain difficulty level on deformable soil (randomized suite, per-episode random friction). The hybrid leads on both metrics at every level; the largest gaps are on flat sand, where the slip-blind baseline is most aggressive.}
\label{fig:mcte_comparison}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_slip_robustness.pdf}
\caption{Performance conditioned on realized mean wheel slip (randomized suite, pooled tertile bounds). The controllers are nearly identical in benign-slip episodes; the gap opens monotonically with slip, the flatter hybrid curves answering RQ3.}
\label{fig:robustness}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig_learning_curve.pdf}
\caption{Training dynamics of the hybrid residual policy on deformable soil over $6.4$ million episodes: rolling training success (top) and normalized residual channel magnitudes (bottom). The residual self-organizes into a steering-dominant corrector ($|\Delta\omega|$ rising to $\approx 0.13$ while $|\Delta v|$ stays near $0.03$ under the effort penalty). [PPO-only curve pending.]}
...
\begin{itemize}
\item \textbf{RQ1 -- Error, and the fidelity gate:} The hybrid achieves lower MCTE than LQR under both contact models, but the magnitude is dictated by contact fidelity: $d_z \approx 0.05$ (millimeters) on rigid contact versus $d_z = 0.30$ and $-47\%$ MCTE on deformable soil, with the tracking advantage present in every geometry, slope, and difficulty cell, and holding in $16$ of $16$ held-out worlds. The residual earns its keep exactly where the environment contains dynamics the linear model cannot represent.
\item \textbf{RQ2 -- Sample efficiency, restated as competence-per-budget:} At a slightly larger step budget ($3.93$ vs.\ $3.3 \times 10^9$), end-to-end PPO never reached hybrid-level \emph{tracking} at any point in training, under either reward variant: the ungated policy froze at $31\%$ curriculum difficulty and converged to the shortcut ($0.137$~m MCTE, $65\%$ above the hybrid, trailing by $23$ points under tracked completion), while the corridor-gated policy learned genuine tracking, out-tracking even pure LQR on flat sand ($0.061$ vs.\ $0.113$~m), but froze at $13\%$ difficulty and collapsed above it ($27.1\%$ sweep completion). The capability is learnable; the reach, at this budget, is not. For the path-tracking competence this work targets, the residual structure is therefore not merely faster to train; it is the difference between acquiring broad on-path competence and choosing between breadth and fidelity \cite{johannink2019residual, mehta2024hybrid}.
...
\item \textbf{RQ5 -- Completion:} On the training-length randomized suite the hybrid completes more episodes at every terrain level. On the deliberately long discrete suite, LQR completes more within the cap because the hybrid trades speed for tracking; under fair path-proportional clocks the ordering persists and the remaining failures are entrapment rather than timeouts, so on adversarial geometries the trade is intrinsic. Across the sixteen-world paired suite, completion is higher in $15/16$ worlds (pooled $+0.6$ points, $+1.4$ in the low-traction stratum), so the completion advantage generalizes. Against end-to-end PPO the completion axis must be read jointly with tracking: PPO posts the highest raw completion by shortcutting the path, and under tracked completion (goal reached with MCTE $\le 0.10$~m) the hybrid leads at every level ($77.1\%$ vs.\ $71.8\%$ LQR and $53.7\%$ PPO). We argue completion, tracking, and their conjunction should all be reported for deformable-terrain controllers, and the conjunction is the mission-relevant one.
\end{itemize}
...
\item \textbf{PPO-only:} with the std clamp and ramped curriculum, training is stable; the failure mode is specification-level rather than dynamical. Freed of a reference-tracking anchor, the progress-dominant reward admits a shortcut optimum: the policy arrives reliably but off-route ($0.137$~m MCTE; $31.8\%$ of its completions exceed the $0.10$~m tracked-completion gate). The LQR baseline is precisely what anchors the hybrid's optimization to the path, which is the structural argument for residual learning in tracking tasks. Corridor-gating the progress reward eliminates the shortcut but produces the complementary failure: an honest tracker that cannot climb the curriculum ($13\%$ ceiling) and, beyond its trained envelope, fails by crawling and parking ($14.5\%$ of sweep episodes) rather than by deserting the path. Both optima stall against the same tracking-quality gate; reward design relocates end-to-end PPO's failure, while the baseline anchor removes it.
...
\subsubsection{Relation to Flight Practice}
Current Mars flight practice brackets these results from the conservative side. Perseverance's onboard autonomy evaluates on the order of a thousand candidate steering arcs per planning cycle with explicit clearance checking, and handles slip not by compensation but by margin: conservative vehicle-envelope inflation, short committed steps with frequent replans, and ground-in-the-loop waypoint selection \cite{verma2023perseverance, nasaACE}. No learned controller has flown in the drive loop. The hybrid architecture studied here is a deliberately small step from that philosophy rather than a departure from it: the certified baseline retains authority, the learned component is bounded to $\pm0.075$~m/s and $\pm0.15$~rad/s of corrective authority, and disabling it recovers the baseline exactly, properties that keep the residual auditable in a way an end-to-end policy is not. Our results quantify what that bounded addition buys on deformable soil (a $16$--$47\%$ tracking improvement and a much flatter slip-degradation curve), which is precisely the regime where margin-based slip handling costs flight vehicles the most drive time.
...
\item Per-episode MCTE is conditioned on the path fraction an episode traverses, so cross-controller MCTE comparisons on suites with very different completion profiles require the jointly-completed conditioning used in Section~\ref{sec:sandresults}; a progress-weighted tracking metric is a systematic refinement.
\item The tracked-completion threshold ($0.10$~m) is a design choice: the hybrid leads for any gate up to $0.15$~m (at a strict $0.08$~m gate the margins widen, $75.3\%$ vs.\ $69.9\%$ vs.\ $32.7\%$), while gates of $0.20$~m and beyond approach PPO's own mean error and stop encoding path adherence at all. A continuous joint metric is a natural refinement.
...
\item Absolute simulated contact behavior is GPU-stack-dependent: a mid-study NVIDIA driver update shifted the slip-blind baseline's cross-track error by roughly $15\%$ at matched friction, protocol, and code. All comparative results reported here were therefore generated with both controllers on an identical software stack, and we caution that absolute (as opposed to paired) PhysX-based soil metrics should not be compared across driver versions.
\item Real-world validation is limited to a single sand-pit environment, and hardware results are pending.
\item The terrain bank contains $12$ distinct height-field patches ($6$ difficulty levels $\times$ $2$ variants), a size set by the PhysX collision-mesh budget at the $0.025$~m facet resolution the $6.25$~cm wheels require, and training and evaluation share this bank. Per-episode diversity comes from the $500$-path bank, continuous per-wheel friction, spawn variation, and the soil field rather than from patch count, and on soil the difficulty axis is demonstrably soil interaction rather than geometry (Section~\ref{sec:sandresults}); nevertheless, per-world terrain diversity is bounded. The sixteen-world paired suite (Section~\ref{sec:sandresults}) shows both metrics generalize across $192$ regenerated patches, $2{,}000$ paths, and scenario-locked traction; the remaining limitation is world-hardness variance across seeds at the highest difficulty, addressed by more variants per row and multi-seed averaging.
...
\item Our end-to-end comparisons are scoped to reward-shaped PPO (ungated and corridor-gated) at a $3.9\times10^9$-step budget. Stronger pure-learning recipes remain untested here: teacher--student privileged distillation, which underlies the strongest deformable-terrain legged results \cite{lee2020learning, choi2023deformable}, and constrained-RL formulations that treat path adherence as a constraint rather than a shaped penalty \cite{achiam2017cpo}, the principled generalization of our corridor gate. Evaluating these against the residual architecture is future work; the present claim is that at matched budget and reward family, the baseline anchor dominates reward repair.
\end{itemize}
...
The central empirical finding is that contact fidelity gates the value of residual learning. On rigid Coulomb contact, the hybrid's advantage over a well-tuned LQR is real but small and localized ($+2.2$ success points; $d_z \approx 0.05$ on tracking), and resists amplification by reward shaping. On deformable soil, the identical architecture reduces mean cross-track error by $47\%$ ($0.073$ vs.\ $0.137$~m), improves success at every terrain difficulty level of the training distribution ($83.4\%$ vs.\ $79.3\%$), degrades far more gracefully with realized slip ($+11.9$ success points and $-59\%$ MCTE in the highest-slip tertile), and tracks better in every cell of a $90{,}000$-episode paired suite ($d_z = 0.30$, $p < 10^{-4}$). Across sixteen held-out worlds under the paired protocol, with unseen paths and scenario-locked per-wheel friction, the hybrid tracks better in every single world ($16/16$, pooled $-16\%$) and completes more in $15$ of $16$, with the tracking gap scaling from exactly zero on high-grip episodes to $-21\%$ under scarce traction, confirming the advantage is a property of the learned slip management rather than of any particular terrain, path, or friction draw. The learned residual self-organizes into a corrective steering controller, and the slip-blind baseline's most dangerous regime turns out to be \emph{flat} sand, a qualitative failure the slip-aware hybrid removes. Alongside these results we documented the training-stability measures residual PPO required at scale (noise-std clamping, a damped effort penalty with baseline-relative credit, uniform rather than ramped terrain difficulty).

Future work proceeds along the path this paper sets up: evaluating stronger pure-learning baselines against the residual architecture (teacher--student privileged distillation \cite{choi2023deformable}, constrained-RL objectives \cite{achiam2017cpo}); calibrating the soil coefficients from physical sand-pit measurements and quantifying sim-to-real retention on the Leo Rover; \textbf{multi-robot extension} to cooperative transport tasks; \textbf{vision integration} for predictive terrain-aware control; \textbf{advanced terrain modeling} via data-driven soil models learned from hardware data; \textbf{formal verification} of hybrid controller bounds; and \textbf{expanded path diversity}. This work provides a foundation for deploying adaptive, learning-enhanced control on planetary rovers while preserving the safety and interpretability required for mission-critical applications.
...
%   choi2023deformable, verma2023perseverance, achiam2017cpo
% ------------------------------------------------------------------
